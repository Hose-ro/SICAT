...
export class CreateMateriaDto {
  ...
  @ApiPropertyOptional({ example: '08:00' }) @IsOptional() @Matches(/^([01]\d|2[0-3]):([0-5]\d)$/) horaInicio?: string;
  @ApiPropertyOptional({ example: '10:00' }) @IsOptional() @Matches(/^([01]\d|2[0-3]):([0-5]\d)$/) horaFin?: string;
  @ApiPropertyOptional({ example: 'Lunes,Miercoles,Viernes' }) @IsOptional() @IsString() dias?: string;
