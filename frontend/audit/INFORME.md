...
Estado verificado sobre `main` el 25 de septiembre de 2026. El audit se cerró con comprobaciones estáticas, unitarias, de build y de navegador; no se desplegó.
...
| 11 | P2 | Tres vocabularios de botón | Corregido | `.btn-*` uiverse: 0; `bg-slate-900`: 0; `<Button>` 326 usos. Los 11 `<button>` nativos restantes son controles específicos con estilos semánticos propios |
...
|----------|------------------|---------|
| Chunk de entrada | `index-K2okgRfO.js` 908,947 B / 241,231 B gzip | `index-*.js` 340,907 B / 113,001 B gzip |
| CSS | 160.35 kB / 27.90 kB gzip | 112.50 kB / 18.96 kB gzip |
| JS inicial completo — ALUMNO `/alumno/horario` | 908,947 B (todo en un chunk) | 505,394 B / 173,336 B gzip (34 scripts) |
| JS inicial completo — DOCENTE `/asistencias` | 908,947 B | 570,198 B / 188,474 B gzip (37 scripts) |
| JS inicial completo — ADMIN `/dashboard` | 908,947 B | 503,243 B / 171,282 B gzip (32 scripts) |
...
| `npm test` (vitest) | 41/41 en 9 archivos, incluidos cancelar/Escape, confirmar una vez + bloqueo y aprobaciones concurrentes |
| `npm run build` | OK, sin aviso de chunk > 500 kB |
| `npm run test:e2e` (Playwright, Chromium) | 98/98 finales: 96 aprobaron en el barrido completo y las 2 expectativas desactualizadas se corrigieron y revalidaron juntas. Incluye login (light/dark × 390/1440 + axe), modal (foco/Tab/Escape/retorno/anidado), contraste renderizado (`audit/contrast.json`: light mín 4.62:1, dark mín 6.1:1), áreas táctiles, horarios, confirmaciones, asistencia, exportaciones/importación, navegación por rol, carga diferida, recuperación de módulos y **76 rutas × tema × ancho** con axe (`audit/rutas-axe.json`): 0 violaciones, 0 overflow, 1 `h1`, 0 errores JS |
